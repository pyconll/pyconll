...
class TestUniqueArray:
    ...
    def test_unique_array_of_custom(self):
        """Test unique_array with custom descriptor elements."""
        desc = unique_array(
            custom(deserialize=lambda s: int(s), serialize=lambda x: str(x)),
            "|",
...
class TestCustom:
    ...
    def test_custom_simple_conversion(self):
        """Test custom with simple conversion functions."""
        desc = custom(deserialize=lambda s: int(s) * 2, serialize=lambda x: str(x // 2))
        assert_conversions(desc, 10, "5")

    def test_custom_complex_type(self):
        """Test custom with a complex type."""
        ...
        desc = custom(deserialize=parse_coords, serialize=format_coords)
        assert_conversions(desc, (1.5, 2.5), "1.5,2.5")

    def test_custom_with_stateful_functions(self):
        """Test custom with functions that have state."""
        prefix = "PREFIX_"

        desc = custom(deserialize=lambda s: prefix + s, serialize=lambda s: s.removeprefix(prefix))
        assert_conversions(desc, "PREFIX_value", "value")

    def test_custom_nested_in_array(self):
