...
def test_ngram_multiple_per_sentence():
    ...
    """
    c = pyconll.load_from_file(fixture_location("long.conll"))
...
def test_ngram_case_insensitive_n_token():
    ...
    c = pyconll.load_from_file(fixture_location("long.conll"))
    s, i, tokens = next(
        find_ngrams(c, "l' orgaNisaTion pour La sécurité et la".split(), case_sensitive=False)
    )
...
def test_no_nonprojectivities():
    ...
    """
    c = pyconll.load_from_file(fixture_location("projectivities.conll"))
    sent = c[0]
...
def test_overlapping_nonprojectivities():
    ...
    assert set(deps) == set([(sent["16"], sent["4"]), (sent["16"], sent["11"])])
...
def test_multiple_nonprojectivities():
    ...
    assert set(deps) == set(
        [(sent["22"], sent["3"]), (sent["22"], sent["21"]), (sent["28"], sent["25"])]
    )
