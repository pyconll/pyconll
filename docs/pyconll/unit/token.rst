...
Since all of these fields are ``dicts``, modifying non existent keys will result in a ``KeyError``. This means that new values must be added as in a normal ``dict``. For ``set`` based ``dicts``, ``feats`` and specific fields of ``misc``, the new key must be assigned to an empty ``set`` to start.
...
misc
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Lastly, for ``misc``, the documentation only specifies that the values are separated by a '|'. So for this reason, the value for ``misc`` is either ``None`` for entries with no '=', and an attribute value pair otherwise with the value being a ``set`` of ``strings``. A key with a value of ``None`` is output as a singleton, while a key with an empty ``set`` is not output.
